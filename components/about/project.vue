						...
						<div v-for="project in projects" :key="project._id">
							<div class="grid grid-cols-1 bg-white border border-gray-200 rounded-lg shadow md:flex-row md:max-w-xl hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
								<div class="grid grid-cols-1">
									<div class="col-span-full">
										<img class="object-cover w-full rounded-t-sm h-auto bg-gray-500 bg-blend-multiply" :src="$urlFor(project.mainImage).url()" :alt="project.mainImage.alt">
									</div>
								</div>
								<div class="grid grid-cols-1">
									<div class="col-span-full">
										<div class="col-span-full md:shrink-0">
											<div class="grid grid-cols-1 p-4 leading-normal">
												<div class="col-span-full">	
													<h5 class="mb-2 text-center text-2xl font-bold tracking-tight text-gray-900 dark:text-white capitalize">
														{{project.title}}
													</h5>
												</div>
											</div>
											...
											<div class="grid grid-cols-1">
												<div class="place-self-start px-4">
													...
													<div class="flex justify-start space-x-2 p-2" v-if="project.members !== null">
														<div v-for="person in persons" :key="person._id">
															<div v-for="member in project.members">
																<img v-if="member.person._ref === person._id" class="w-8 h-8 mb-3 rounded-full shadow-lg" :src="$urlFor(person.image).url()" :alt="person.image.alt"/>
															</div>
														</div>
													</div>
												</div>
								...
								<div class="grid grid-cols-1 p-6 mb-2">
									<div class="place-self-center">
										<a :href="`/project/${project?.slug?.current}`" type="button" class="max-w-full w-full text-center text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2">
